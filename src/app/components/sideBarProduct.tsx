...
export default function SideBarProduct() {
    ...
    return (
        <>
            ...
            {isOpen && (
                <div className="fixed top-20 left-0 w-1/4 h-auto bg-[#e0d6cb] pl-10 z-30">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="cursor-pointer hover:scale-110 transition-all duration-300 size-6 absolute text-[#a36e3a] top-15 right-4 bg-white p-1 rounded-full z-10" onClick={() => setIsOpen(!isOpen)}>
                    ...
                    <div className="font-cormorant flex flex-col mt-20 p-4 relative z-10">
                        ...
                        <Link href="/adboutiquevillas/services" className={`font-cormorant italic text-4xl mb-8 transition-all duration-300 hover:scale-105 ${pathname.startsWith('/adboutiquevillas/accommodations') ? 'text-[#a36e3a]' : 'text-gray-800'}`} style={{ fontWeight: '300', letterSpacing: '1px' }}>
                            Services
                        </Link>
